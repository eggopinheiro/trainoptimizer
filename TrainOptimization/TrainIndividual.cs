...
public class TrainIndividual : IIndividual<Gene>, IComparable<IIndividual<Gene>>
{
    ...
    */
    ...
    private Dictionary<int, Gene[]> mStopLocationArrival = null;
    ...
    public TrainIndividual(List<Gene> pGenes, int pUniqueId, Dictionary<int, int> pDicDistRef, IFitness<Gene> pFitness, DateTime pDateRef, Dictionary<Int64, List<Trainpat>> pPATs, Random pRandom) : this(pFitness, pDateRef, pGenes, pPATs, pRandom)
    {
        mUniqueId = pUniqueId;
        mDicDistRef = pDicDistRef;
    }

    public TrainIndividual(IFitness<Gene> pFitness, DateTime pDateRef, List<Gene> pTrainList, Dictionary<Int64, List<Trainpat>> pPATs, Random pRandom)
    {
        mFitness = pFitness;
        mList = new List<Gene>();
        mDicTrain = new Dictionary<Int64, Gene>();
        ...
        try
        {
            ...
            mStopLocationArrival = new Dictionary<int, Gene[]>();
            foreach (StopLocation lvStopLocation in StopLocation.GetList())
            {
                mStopLocationDeparture.Add(lvStopLocation.Location, new Gene[lvStopLocation.Capacity * StopLocation.SIDES]);
                mStopLocationArrival.Add(lvStopLocation.Location, new Gene[lvStopLocation.Capacity * StopLocation.SIDES]);
            }
    ...
    public IIndividual<Gene> Clone()
    {
        IIndividual<Gene> lvRes = new TrainIndividual(mList, mUniqueId, mDicDistRef, mFitness, mDateRef, mPATs, mRandom);
    ...
    public Dictionary<int, Gene[]> GetStopLocationDeparture()
    {
        return mStopLocationDeparture;
    }

    public Dictionary<int, Gene[]> GetStopLocationArrival()
    {
    ...
    public List<Gene> GetGenes(int pStartIndex, int pEndIndex)
    {
        ...
        while (lvGene.State != Gene.STATE.IN)
        {
            lvIndex++;
            lvGene = mList[lvIndex];
    ...
    public void AddGenes(List<Gene> pGenes, bool pNeedUpdate = true)
    {
            ...
            if (pGenes != null)
            {
                if (pNeedUpdate)
                {
                    foreach (Gene lvGene in pGenes)
                    {
                        ...
                        if ((lvGene.StopLocation != null) && (lvGene.StopLocation.Location == lvGene.EndStopLocation.Location))
                        {
                        ...
                        else
                        {
                            if (lvGene.State == Gene.STATE.OUT)
                            {
                                ...
                                {
                                    lvGenesStopLocation = mStopLocationDeparture[lvGene.StopLocation.Location];
                            ...
                            else if (lvGene.State == Gene.STATE.IN)
                            {
                                if (lvGene.StopLocation != null)
                                {
                                    lvGenesStopLocation = mStopLocationArrival[lvGene.StopLocation.Location];
                                    if (lvGene.Track <= lvGene.StopLocation.Capacity)
                                    {
                                        /* 
                                        * Gene[] em mStopLocationDeparture e mStopLocationArrival deve ser usada a formula
                            ...
                            {
                                DebugLog.Logar("Problema em AddGenes", false, pIndet: TrainIndividual.IDLog);
                            }
                            ...
                            if (!mDicTrain.ContainsKey(lvGene.TrainId))
                            {
                                if (lvGene.Speed == 0.0)
                                {
                                    lvGene.Speed = mMinSpeedLimit;
                                }

                                if (lvGene.HeadWayTime <= lvGene.Time)
                                {
                                    lvGene.HeadWayTime = lvGene.Time.AddHours(mTrainLenKM / lvGene.Speed);
                                }
                        ...
                        if (mCalcDistRef)
                        {
                            ...
                            if (!mDicDistRef.ContainsKey(lvGene.GetID()))
                            {
                                mDicDistRef.Add(lvGene.GetID(), mList.Count);
                            }
                        }
                        mList.Add(lvGene);
                    }
    ...
    public int GetDistanceFrom(IIndividual<Gene> pIndividual)
    {
        int lvRes = -1;
        int lvPositionID;
        int lvValue = -1;
    ...
    private void RemoveFromStopLocation(Gene pGene)
    {
        ...
        if (pGene.StopLocation != null)
        {
            ...
            if (lvListGeneStopLocation.Contains(pGene.TrainId))
            {
                lvListGeneStopLocation.Remove(pGene.TrainId);

#if DEBUG
                if (DebugLog.EnableDebug)
                {
                    DebugLog.Logar("Removendo Gene (" + pGene + ") da Stop Location (" + pGene.StopLocation + ")", pIndet: TrainIndividual.IDLog);
                }
#endif
            }
        }
        ...
        if (DebugLog.EnableDebug)
        {
            ...
            if (lvTrainId == -1)
            {
                for (int i = 0; i < mList.Count; i++)
                {
                    ...
                    lvRes.Append("(");
                    lvRes.Append(mList[i].TrainId);
                    lvRes.Append(" - " + mList[i].TrainName + ") ");
                    lvRes.Append(mList[i].TrainName);
                    lvRes.Append(") ");

                    if (mList[i].State == Gene.STATE.OUT)
                    {
                        lvRes.Append(mList[i].SegmentInstance.Location);
                        lvRes.Append(".");
                        lvRes.Append(mList[i].SegmentInstance.SegmentValue);
                        lvRes.Append(" (Saida: ");
                        lvRes.Append(mList[i].Time);
                        lvRes.Append(")");
                    }
                    else if(mList[i].State == Gene.STATE.IN)
                    {
                        lvRes.Append(mList[i].SegmentInstance.Location);
                        lvRes.Append(".");
                        lvRes.Append(mList[i].SegmentInstance.SegmentValue);
    ...
    private void DumpStopArrivalLocation(StopLocation pStopLocation)
    {
        Gene[] lvGenes = null;
        ...
        if (DebugLog.EnableDebug)
        {
            ...
            if (pStopLocation != null)
            {
                DebugLog.Logar(" ----------------------------  DumpStopArrivalLocation (" + pStopLocation + ") ------------------------------------ ", pIndet: TrainIndividual.IDLog);
    ...
    public void DumpCurrentState(string pStrInfo, int pUniqueId = -1)
    {
        Dictionary<Int64, Gene> lvGenePosition = null;
        int lvUniqueId = mUniqueId;

        if (DebugLog.EnableDebug)
        {
            if(pUniqueId != -1)
            {
                lvUniqueId = pUniqueId;
            }
    ...
    public void DumpCurrentPosDic(string pStrInfo, int pUniqueId = -1)
    {
        int lvUniqueId = mUniqueId;
    ...
    public bool Save()
    {
        ...
        foreach (Gene lvGene in mList)
        {
            ...
            PlanOptDataAccess.Insert(lvGene.TrainId, lvGene.TrainName, ConnectionManager.GetUTCDateTime(lvGene.Time), lvPosition, lvGene.Track, SegmentDataAccess.Branch);
        }
    ...
    public void BranchAndBound(IIndividual<Gene> pRefIndividual, List<Gene> pTrainList, List<Gene> pPlanList)
    {
        ...
        List<Gene> lvBestIndividual = ((TrainIndividual)pRefIndividual).GetGenes();
        ...
        for (int i = 0; i < pTrainList.Count; i++)
        {
            ...
            {
                ProcessGene(pTrainList[i], lvCurrentTime);
            }
        }
        ...
        */
        ...
        {
            lvRes = ProcessGene(lvGene, lvCurrentTime);
    ...
    private List<Gene> BackTrack(List<Gene> pPlanList, DateTime pCurrentTime)
    {
        ...
        foreach (Gene lvGene in lvTrains)
        {
            lvProcessGeneResult = ProcessGene(lvGene, lvCurrentTime);
    ...
    private bool VerifyLoadedData(List<Gene> ploadedList, List<Gene> pMovList, List<Gene> pPlanList, out HashSet<Int64> pLoadedSet, out HashSet<Int64> pPlanSet)
    {
        bool lvRes = false;
        ...
        if ((ploadedList != null) && (ploadedList.Count > 0))
        {
            ...
            foreach (Gene lvGen in ploadedList)
            {
                if (!pLoadedSet.Contains(lvGen.TrainId))
                {
                    pLoadedSet.Add(lvGen.TrainId);
                }
            }

            foreach (Gene lvGen in pMovList)
            {
                if (!pLoadedSet.Contains(lvGen.TrainId))
                {
                    lvRes = false;
            ...
            foreach (Gene lvGen in pPlanList)
            {
                if (!pLoadedSet.Contains(lvGen.TrainId))
                {
                    lvRes = false;
                ...
                if(!pPlanSet.Contains(lvGen.TrainId))
                {
                    pPlanSet.Add(lvGen.TrainId);
                }
            }
        }
    ...
    private void AddFromQueue(Queue<Gene> pQueue, DateTime pCurrentTime)
    {
        Gene lvGene = null;
        bool lvRes;
        ...
        for (int i = 0; i < pQueue.Count; i++)
        {
            ...
            lvRes = ProcessGene(lvGene, pCurrentTime);
            if (!lvRes)
            {
    ...
    private int LoadedDataComparer(List<Gene> pLoadedData)
    {
        int lvResPos = -1;
        Gene lvGene = null;
        ...
        for(int i = 0; i < mList.Count; i++)
        {
            ...
            if((lvGene.TrainId == pLoadedData[i].TrainId) && (lvGene.StopLocation != null) && (lvGene.StopLocation.Location == pLoadedData[i].StopLocation.Location))
            {
                lvResPos = i;
            }
            else
    ...
    public bool GenerateIndividual(List<Gene> pPlanList, int pUniqueId = 0, bool pAllowDeadLockIndividual = false)
    {
        bool lvRes = false;
        bool lvProcessGeneResult;
        List<Gene> lvMovTurn = null;
        List<Gene> lvPlans = null;
        List<Gene> lvLoadedData = null;
        HashSet<Int64> lvLoadedSet = null;
        HashSet<Int64> lvPlannedSet = null;
        Queue<Gene> lvQueue = null;
        Gene lvGene = null;
        ...
        List<Gene> lvRefLoadedData = null;
        ...
        try
        {
            ...
            if (pUniqueId != 0)
            {
                ...
                {
                    lvQueue = new Queue<Gene>();
                }

                lvRefLoadedData = new List<Gene>(lvLoadedData);
            }
            ...
            */
            ...
            {
                if (lvLoadedDataStatus && (lvLoadedData.Count > 1))
                {
                    if((lvLoadedData[0].StopLocation != null) && (lvLoadedData[0].TrainId == lvLoadedData[1].TrainId))
                    {
                        lvGene = lvLoadedData[0];

                        if (mDicTrain.ContainsKey(lvGene.TrainId))
                        {
                            lvGene = mDicTrain[lvGene.TrainId];
                        }
                        else
                        {
                            if(!lvPlannedSet.Contains(lvGene.TrainId))
                            {
                                lvGene = null;
                            }
                        }
                        ...
                        if (lvGene != null)
                        {
                            ...
                            else
                            {
                                if (lvPlannedSet.Contains(lvGene.TrainId))
                                {
                                    lvPlannedSet.Remove(lvGene.TrainId);

                                    for(int i = 0; i < lvPlans.Count; i++)
                                    {
                                        lvGene = lvPlans[i];
                                        if(lvGene.TrainId == lvLoadedData[0].TrainId)
                                        {
                                            lvPlans.RemoveAt(i);
                                            break;
                                        }
                                ...
                                {
                                    lvPlans = new List<Gene>();
                                }
                            }
                        }
                    }

                    if(lvLoadedData.Count > 1)
                    {
                        if(lvLoadedData[0].TrainId == lvLoadedData[1].TrainId)
                        {
                ...
                else
                {
                    ...
                    while (lvMovTurn.Count > 0)
                    {
                        ...
                        foreach (Gene lvGen in lvMovTurn)
                        {
                            //lvTotalValue += Convert.ToInt32(GetOptTimeToEnd(lvGen.TrainId) * 100);
                        ...
                        {
                            //lvTotal += Convert.ToInt32(GetOptTimeToEnd(lvMovTurn[i].TrainId) * 100);
                            ...
                            if (lvTotal >= lvRandomValue)
                            {
                                if ((lvLoadedData != null) && (lvLoadedData.Count > 0))
                                {
                                    if(lvLoadedSet.Contains(lvMovTurn[i].TrainId))
                                    {
                                        lvGene = lvLoadedData[0];
                    ...
                    for (int i = 0; i < lvPlans.Count; i++)
                    {
                        if (!mDicTrain.ContainsKey(lvPlans[i].TrainId))
                        {
                            lvProcessGeneResult = ProcessGene(lvPlans[i], lvCurrentTime);

                            if (lvProcessGeneResult)
                            {
                                if ((lvPlannedSet != null) && (lvPlannedSet.Contains(lvPlans[i].TrainId)))
                                {
                                    lvPlannedSet.Remove(lvPlans[i].TrainId);
                                }
                                lvPlans.RemoveAt(i);
                                //DebugLog.Logar("GenerateIndividual().VerifyConflict() = " + VerifyConflict(), pIndet: TrainIndividual.IDLog);
                            }
    ...
    private void DumpDicTrain(Dictionary<Int64, Gene> pDicTrain)
    {
    ...
    private DateTime GetCurrentFirstOutputTime(Gene pGene, StopLocation pStopLocation, int pDirection)
    {
        ...
        {
            foreach (Gene lvGene in mStopLocationDeparture[lvStopLocationValue])
            {
                if (lvGene == null)
                {
                ...
                else
                {
                    if (pGene.TrainId != lvGene.TrainId)
                    {
                        if (lvRes == DateTime.MinValue)
                        {
                            ...
                            if (DebugLog.EnableDebug)
                            {
                                DebugLog.Logar("Atraso do trem " + pGene.TrainId + " Em " + pGene.SegmentInstance.Location + "." + pGene.SegmentInstance.SegmentValue + " com tempo anteior de " + pGene.Time + " foi para " + lvRes + " por lvLastDepTime (" + lvGene + ")", pIndet: TrainIndividual.IDLog);
                            }
#endif
                            //DumpStopDepLocation(pStopLocation);
                        }
                        else if ((lvGene.Time < lvRes) && (lvGene.HeadWayTime != DateTime.MinValue))
                        {
                            lvRes = lvGene.HeadWayTime;
    ...
    private DateTime GetHeadWay(Gene pGene, StopLocation pStopLocation, int pDirection)
    {
        ...
        if (pStopLocation != null)
        {
            ...
            if (DebugLog.EnableDebug)
            {
                DumpStopArrivalLocation(pStopLocation);
            }
#endif

            if (mStopLocationArrival.ContainsKey(lvStopLocationValue))
            {
                foreach (Gene lvGene in mStopLocationArrival[lvStopLocationValue])
                {
                    if (lvGene != null)
                    {
                        if (pGene.TrainId == lvGene.TrainId)
                        ...
                        else
                        {
                            if ((lvGene.HeadWayTime > lvRes) && (pDirection != lvGene.Direction))
                            {
                                ...
                                if (DebugLog.EnableDebug)
                                {
                                    ...
                                    lvStrInfo.Append("GetHeadWay => lvCurrentTime após HeadWayTime de (");
                                    lvStrInfo.Append(lvGene.TrainId);
                                    lvStrInfo.Append(" - ");
                                    lvStrInfo.Append(lvGene.TrainName);
                                    lvStrInfo.Append(" em ");
                ...
                if ((lvRes == DateTime.MinValue) && (lvMyHeadWay != DateTime.MinValue))
                {
                    ...
                    if (DebugLog.EnableDebug)
                    {
                        DebugLog.Logar("GetHeadWay => lvCurrentTime após MyHeadWay para Gene (" + pGene.TrainId + " - " + pGene.TrainName + " em (" + pStopLocation + "), considerando Direction: " + pDirection + ") = " + lvRes, pIndet: TrainIndividual.IDLog);
                        DumpGene(pGene);
                        DumpStopLocation(pStopLocation);
                    }
#endif
    ...
    private bool[] VerifySegmentDeadLock(Gene pGene, StopLocation pNextStopLocation, Segment pNextSwitch, out bool pHasSameDirection, out int pSumDir, out bool[] pLastOcup)
    {
        ...
        if (DebugLog.EnableDebug)
        {
            DebugLog.Logar("\n ------------------------- VerifySegmentDeadLock para (" + pGene + ") -------------------------------------- ", pIndet: TrainIndividual.IDLog);
        }
#endif
        ...
        if (lvHasSameDirection && (pNextSwitch != null))
        {
            ...
            else
            {
                ...
                else
                {
                    if (pGene.Direction > 0)
                    {
                        lvResSameDir = VerifyNextSegment(pGene, lvStopLocation, Segment.GetCurrentSegment(lvLimitPosition, -1, 1, out lvIndex), false, out lvHasSameDirection, out lvLastSumSameDir, out lvLastOccup, out lvLastCount);
                    }
                    else
                    {
                        lvResSameDir = VerifyNextSegment(pGene, lvStopLocation, Segment.GetCurrentSegment(lvLimitPosition, 1, 1, out lvIndex), false, out lvHasSameDirection, out lvLastSumSameDir, out lvLastOccup, out lvLastCount);
    ...
    private void SetHeadwaySetTime(Gene pGene, StopLocation pStopLocation, StopLocation pNextStopLocation, ref List<DateTime> pHeadwayTime, bool pOnlyDeparture = false)
    {
        DateTime lvCurrentTime = pGene.HeadWayTime;
        ...
        if (DebugLog.EnableDebug)
        {
            ...
            lvStrInfo.Clear();
            lvStrInfo.Append(" ------------------------------ SetHeadwaySetTime  ------------------------------ ");
        ...
        if (DebugLog.EnableDebug)
        {
            ...
            lvStrInfo.Clear();
            lvStrInfo.Append("SetHeadwaySetTime.pOnlyDeparture = ");
            lvStrInfo.Append(pOnlyDeparture);
        ...
        if (!pOnlyDeparture)
        {
            if (pStopLocation != null)
            {
                lvCurrentTime = GetHeadWay(pGene, pStopLocation, pGene.Direction);

                if(lvCurrentTime > pGene.HeadWayTime)
                {
                    pHeadwayTime.Add(lvCurrentTime);
                    ...
                    if (DebugLog.EnableDebug)
                    {
                        ...
                        lvStrInfo.Clear();
                        lvStrInfo.Append("lvCurrentTime em pHeadwayTime do Gene (");
                        lvStrInfo.Append(pGene.TrainId);
                        lvStrInfo.Append(" - ");
                        lvStrInfo.Append(pGene.TrainName);
                        lvStrInfo.Append(") = ");
            ...
            lvNextHeadWay = GetHeadWay(pGene, pNextStopLocation, pGene.Direction * (-1));

            if (lvCurrentTime > pGene.HeadWayTime)
            {
                pHeadwayTime.Add(lvNextHeadWay);
                ...
                if (DebugLog.EnableDebug)
                {
                    ...
                    lvStrInfo.Clear();
                    lvStrInfo.Append("lvNextHeadWay em pHeadwayTime do Gene (");
                    lvStrInfo.Append(pGene.TrainId);
                    lvStrInfo.Append(" - ");
                    lvStrInfo.Append(pGene.TrainName);
                    lvStrInfo.Append(") = ");
                    lvStrInfo.Append(lvNextHeadWay);
                    lvStrInfo.Append(" por ser maior que o headway atual ");
                    lvStrInfo.Append(pGene.HeadWayTime);
        ...
        if (pGene.Time > pGene.HeadWayTime)
        {
            pHeadwayTime.Add(pGene.Time);
            ...
            if (DebugLog.EnableDebug)
            {
                ...
                lvStrInfo.Clear();
                lvStrInfo.Append("pGene.Time em pHeadwayTime do Gene (");
                lvStrInfo.Append(pGene.TrainId);
                lvStrInfo.Append(" - ");
                lvStrInfo.Append(pGene.TrainName);
                lvStrInfo.Append(") = ");
        ...
        else if (pGene.DepartureTime > pGene.HeadWayTime)
        {
            pHeadwayTime.Add(pGene.HeadWayTime);
            ...
            if (DebugLog.EnableDebug)
            {
                ...
                lvStrInfo.Clear();
                lvStrInfo.Append("pGene.DepartureTime em pHeadwayTime do Gene (");
                lvStrInfo.Append(pGene.TrainId);
                lvStrInfo.Append(" - ");
                lvStrInfo.Append(pGene.TrainName);
                lvStrInfo.Append(") = ");
                lvStrInfo.Append(pGene.DepartureTime);
                lvStrInfo.Append(" por ser maior que o headway atual ");
                lvStrInfo.Append(pGene.HeadWayTime);
        ...
        lvLastDepTime = GetCurrentFirstOutputTime(pGene, pNextStopLocation, pGene.Direction);

        if (lvLastDepTime > pGene.HeadWayTime)
        {
            pHeadwayTime.Add(lvLastDepTime);
#if DEBUG
            if (DebugLog.EnableDebug)
            {
                StringBuilder lvStrInfo = new StringBuilder();
                ...
                lvStrInfo.Clear();
                lvStrInfo.Append("lvLastDepTime em pHeadwayTime do Gene (");
                lvStrInfo.Append(pGene.TrainId);
                lvStrInfo.Append(" - ");
                lvStrInfo.Append(pGene.TrainName);
                lvStrInfo.Append(") = ");
        ...
        if (DebugLog.EnableDebug)
        {
            ...
            lvStrInfo.Clear();
            lvStrInfo.Append(" ------------------------------ Fim de SetHeadwaySetTime  ----------------------------- ");
    ...
    public void LoadDistRef()
    {
        ...
        for(int i = 0; i < mList.Count; i++)
        {
            ...
            lock(mDicDistRef)
            {
                if (!mDicDistRef.ContainsKey(lvGene.GetID()))
                {
                    mDicDistRef.Add(lvGene.GetID(), i);
                }
            }
        }
    }
    ...
    private Gene GetLastStep(Gene pGene, out int pOutIndex, Gene.STATE pState = Gene.STATE.UNDEF, StopLocation pStopLocation = null, int pInitialIndex=-1)
    {
        ...
        for(int i = lvIndex; i >= 0; i--)
        {
            ...
            if ((lvRes.TrainId == pGene.TrainId) && ((lvRes.State == pState) || (pState == Gene.STATE.UNDEF)) && ((pStopLocation == null) || (lvRes.StopLocation == pStopLocation)))
            {
                pOutIndex = i;
                break;
            }
    ...
    private void UpdateCrossingFail(Gene pRefGene, int pIndex = -1)
    {
        TrainPerformanceControl lvTrainPerformance = null;
        Gene lvOtherTrainGene = null;
        Gene lvRefGene = null;
        Gene lvPrevGene;
        ...
        if (DebugLog.EnableDebug)
        {
            DebugLog.Logar("UpdateCrossingFail.lvRefGene = " + lvRefGene, pIndet: TrainIndividual.IDLog);
            DebugLog.Logar("UpdateCrossingFail.mList.Count = " + mList.Count, pIndet: TrainIndividual.IDLog);
        }
#endif

        if ((lvRefGene == null) || (lvRefGene.StopLocation == null)) return;

        if (!GetTransitInfo(lvRefGene, out lvPrevGene, out lvDistance, out lvPrevGeneIndex, -1)) return;

        if (lvRefGene.State == Gene.STATE.IN)
        {
            for (int ind = pIndex + 1; ind >= 0; ind--)
            {
                lvOtherTrainGene = mList[ind];

                if ((lvOtherTrainGene.StopLocation != null) && (lvRefGene.StopLocation.Location == lvOtherTrainGene.StopLocation.Location) && (lvRefGene.TrainId != lvOtherTrainGene.TrainId) && (lvOtherTrainGene.State == Gene.STATE.OUT))
                {
#if DEBUG
                    if (DebugLog.EnableDebug)
                    {
                        DebugLog.Logar("UpdateCrossingFail.lvOtherTrainGene option 1 = " + lvOtherTrainGene, pIndet: TrainIndividual.IDLog);
                    }
#endif

                    if ((lvOtherTrainGene.Time < lvRefGene.HeadWayTime) && (lvOtherTrainGene.Time > lvPrevGene.Time) && (lvPrevGene.StopLocation.HasBetweenSwitch(lvRefGene.StopLocation)))
                    {
                        lvNextGene = GetLastStep(lvOtherTrainGene, out lvPrevGeneIndex, Gene.STATE.IN, lvPrevGene.StopLocation, -1);
                        lvTrainPerformance = TrainPerformanceControl.GetElementByKey(lvOtherTrainGene.TrainName.Substring(0, 1), lvOtherTrainGene.Direction, lvOtherTrainGene.SegmentInstance.Location, lvOtherTrainGene.SegmentInstance.SegmentValue, lvOtherTrainGene.Track);

#if DEBUG
                        if (DebugLog.EnableDebug)
                        {
                            DebugLog.Logar("UpdateCrossingFail.lvNextGene = " + lvNextGene, pIndet: TrainIndividual.IDLog);
                        }
#endif

                        lvOtherTrainGene.Time = lvRefGene.HeadWayTime;
                        lvOtherTrainGene.Speed = mMinSpeedLimit;
                        lvOtherTrainGene.HeadWayTime = lvOtherTrainGene.Time.AddHours(mTrainLenKM / lvOtherTrainGene.Speed);

#if DEBUG
                        if (DebugLog.EnableDebug)
                        {
                            DebugLog.Logar("UpdateCrossingFail.lvOtherTrainGene atualizado = " + lvOtherTrainGene, pIndet: TrainIndividual.IDLog);
                        }
#endif
                        ...
                        if (lvTrainPerformance == null)
                        {
                            ...
                            lvNextGene.Time = lvOtherTrainGene.Time.AddHours(lvSpentTime);
                            lvNextGene.HeadWayTime = lvOtherTrainGene.HeadWayTime.AddHours(lvSpentTime);
                        ...
                        else
                        {
                            ...
                            {
                                lvNextGene.Time = lvOtherTrainGene.Time.AddHours(lvTrainPerformance.TimeStop / 60.0);
                                lvNextGene.HeadWayTime = lvNextGene.Time.AddHours(lvTrainPerformance.TimeHeadWayStop / 60.0);
                            }
                        ...
                        lvGene = GetLastStep(lvNextGene, out lvPrevGeneIndex, Gene.STATE.OUT, lvNextGene.StopLocation, -1);

                        if(lvGene.Time < lvNextGene.Time)
                        {
                            UpdateCrossingFail(lvGene, lvPrevGeneIndex);
                        }

                        lvGene = GetLastStep(lvOtherTrainGene, out lvPrevGeneIndex, Gene.STATE.IN, lvOtherTrainGene.StopLocation, ind - 1);
                        UpdateCrossingFail(lvNextGene, lvPrevGeneIndex);
                        //GenerateFlotFiles(DebugLog.LogPath);
                    }
                }
            }
        }
        else if(lvRefGene.State == Gene.STATE.OUT)
        {
            if(lvRefGene.Time < lvPrevGene.Time)
            {
                if (lvDistance < mTrainLen)
                {
                    lvMeanSpeed = (lvRefGene.Speed + lvRefGene.Speed * 0.35);
                }
                else
                {
                    lvMeanSpeed = (lvRefGene.Speed + mVMA) / 2.0;
                }

                lvSpentTime = (lvDistance / 100000.0) / lvMeanSpeed;
                lvRefGene.Time = lvPrevGene.Time.AddHours(lvSpentTime);
                lvRefGene.HeadWayTime = lvRefGene.Time.AddHours(mTrainLenKM / lvMeanSpeed);
                lvRefGene.Speed = lvMeanSpeed;

#if DEBUG
                if (DebugLog.EnableDebug)
                {
                    DebugLog.Logar("lvRefGene.State == Gene.STATE.OUT", pIndet: TrainIndividual.IDLog);
                    DebugLog.Logar("UpdateCrossingFail.lvRefGene lvDistance used = " + lvDistance, pIndet: TrainIndividual.IDLog);
                    DebugLog.Logar("UpdateCrossingFail.lvRefGene atualizado = " + lvRefGene, pIndet: TrainIndividual.IDLog);
                }
#endif

                lvNextStopLocation = lvRefGene.StopLocation.GetNextStopSegment(lvRefGene.Direction);
                lvNextGene = GetLastStep(lvRefGene, out lvPrevGeneIndex, Gene.STATE.IN, lvNextStopLocation, -1);

                if (lvNextStopLocation != null)
                {
                    if (lvRefGene.Direction > 0)
                    {
                        lvInitCoordinate = lvRefGene.StopLocation.End_coordinate;
                        lvEndCoordinate = lvNextGene.StopLocation.Start_coordinate;
                    }
                    else
                    {
                        lvInitCoordinate = lvRefGene.StopLocation.Start_coordinate;
                        lvEndCoordinate = lvNextGene.StopLocation.End_coordinate;
                    }
                }
                else
                {
                ...
                if (DebugLog.EnableDebug)
                {
                    DebugLog.Logar("UpdateCrossingFail.lvNextStopLocation = " + lvNextStopLocation, pIndet: TrainIndividual.IDLog);
                    DebugLog.Logar("UpdateCrossingFail.lvNextGene = " + lvNextGene, pIndet: TrainIndividual.IDLog);
                    DebugLog.Logar("UpdateCrossingFail.lvInitCoordinate = " + lvInitCoordinate, pIndet: TrainIndividual.IDLog);
                    DebugLog.Logar("UpdateCrossingFail.lvEndCoordinate = " + lvEndCoordinate, pIndet: TrainIndividual.IDLog);
                    DebugLog.Logar("UpdateCrossingFail.lvDistance = " + lvDistance, pIndet: TrainIndividual.IDLog);
                }
#endif
                ...
                if (lvTrainPerformance == null)
                {
                    lvMeanSpeed = (lvRefGene.Speed + mVMA) / 2.0;
                    lvSpentTime = (lvDistance / 100000.0) / lvMeanSpeed;
                    lvNextGene.Time = lvRefGene.Time.AddHours(lvSpentTime);
                    lvNextGene.HeadWayTime = lvNextGene.Time.AddHours(mTrainLenKM / lvMeanSpeed);
                    lvNextGene.Speed = lvMeanSpeed;

#if DEBUG
                    if (DebugLog.EnableDebug)
                    {
                        DebugLog.Logar("UpdateCrossingFail.lvMeanSpeed = " + lvMeanSpeed, pIndet: TrainIndividual.IDLog);
                        DebugLog.Logar("UpdateCrossingFail.lvNextGene atualizado = " + lvNextGene, pIndet: TrainIndividual.IDLog);
                    }
#endif
                }
                else
                {
                    if (lvPrevGene.Speed > mMinSpeedLimit)
                    {
                        ...
                        {
                            lvNextGene.Speed = (lvRefGene.Speed + ((lvDistance / 100000.0) / (lvTrainPerformance.TimeMovStop / 60.0))) / 2.0;
                            lvNextGene.Time = lvRefGene.Time.AddHours(lvTrainPerformance.TimeMovStop / 60.0);
                            lvNextGene.HeadWayTime = lvRefGene.Time.AddHours(lvTrainPerformance.TimeHeadwayMovStop / 60.0);
                        }
                    ...
                    else
                    {
                        ...
                        {
                            lvNextGene.Speed = (lvRefGene.Speed + ((lvDistance / 100000.0) / (lvTrainPerformance.TimeStopStop / 60.0))) / 2.0;
                            lvNextGene.Time = lvRefGene.Time.AddHours(lvTrainPerformance.TimeStopStop / 60.0);
                            lvNextGene.HeadWayTime = lvRefGene.Time.AddHours(lvTrainPerformance.TimeHeadwayStopStop / 60.0);
                        }
    ...
    public Gene VerifyConflict()
    {
        Gene lvRes = null;
        Gene lvGeneRef = null;
        Gene lvGeneRefPrev = null;
        Dictionary<int, HashSet<Gene>> lvDic = new Dictionary<int, HashSet<Gene>>();
        Dictionary<int, HashSet<Gene>> lvDicIn = new Dictionary<int, HashSet<Gene>>();
        ...
        try
        {
            for (int i = 1; i < mList.Count; i++)
            {
                ...
                if (lvGeneRef.State == Gene.STATE.OUT)
                {
                    if (!lvDic.ContainsKey(lvGeneRef.StopLocation.Location))
                    {
                        lvDic.Add(lvGeneRef.StopLocation.Location, new HashSet<Gene>());
                    }
                    lvDic[lvGeneRef.StopLocation.Location].Add(lvGeneRef);

                    if (lvDicIn.ContainsKey(lvGeneRef.StopLocation.Location))
                    {
                        foreach(Gene lvGene in lvDicIn[lvGeneRef.StopLocation.Location])
                        {
                            if((lvGene.TrainId == lvGeneRef.TrainId) && (lvGene.StopLocation.Location == lvGeneRef.StopLocation.Location))
                            {
                                if(lvGeneRef.Time < lvGene.Time)
                                {
                                    lvRes = lvGeneRef;
                                    //GenerateFlotFiles(DebugLog.LogPath);
                                    return lvRes;
                        ...
                        lvDicIn.Remove(lvGeneRef.StopLocation.Location);
                    }
                }

                if ((lvGeneRef.State == Gene.STATE.IN) && (i > 2))
                {
                    lvGeneRefPrev = mList[i - 1];

                    if (!lvDicIn.ContainsKey(lvGeneRef.StopLocation.Location))
                    {
                        lvDicIn.Add(lvGeneRef.StopLocation.Location, new HashSet<Gene>());
                    }
                    lvDicIn[lvGeneRef.StopLocation.Location].Add(lvGeneRef);

                    if (lvDic.ContainsKey(lvGeneRef.StopLocation.Location))
                    {
                        foreach (Gene lvGene in lvDic[lvGeneRef.StopLocation.Location])
                        {
                            if ((lvGene.Time > lvGeneRefPrev.Time) && (lvGene.Time < lvGeneRef.Time) && (lvGeneRefPrev.StopLocation.HasBetweenSwitch(lvGeneRef.StopLocation)) && (lvGeneRef.Direction != lvGene.Direction))
                            {
                                lvRes = lvGeneRef;
                                //GenerateFlotFiles(DebugLog.LogPath);
                                return lvRes;
    ...
    private void UpdateLastArrival(Gene pGene)
    {
        ...
        try
        {
            ...
            UpdateCrossingFail(pRefGene: pGene);
    ...
    public bool ProcessGene(Gene pGene, DateTime pInitialTime = default(DateTime), bool pUpdate = true)
    {
        ...
        DateTime lvDepTime;
        DateTime lvArrTime;
        List<DateTime> lvHeadwaysTime = null;
        DateTime lvLastDepTime = DateTime.MinValue;
        double lvMeanSpeed = 0.0;
        ...
        if(mTrainFinished.Contains(pGene.TrainId))
        {
            if (mDicTrain.ContainsKey(pGene.TrainId))
            {
        ...
        if (DebugLog.EnableDebug)
        {
            ...
            lvStrInfo.Append(" => ");
            lvStrInfo.Append("ProcessGene( ");
            lvStrInfo.Append(pGene.ToString());
        ...
        try
        {
            if (mDicTrain.ContainsKey(pGene.TrainId))
            {
                ...
                if (DebugLog.EnableDebug)
                {
                    ...
                    lvStrInfo.Clear();
                    lvStrInfo.Append("ProcessGene => Gene obtido de mDicTrain(");
                    lvStrInfo.Append(lvGene.ToString());
                    lvStrInfo.Append(")");
            ...
            else
            {
                ...
                else if (pGene.StopLocation.Location == pGene.StartStopLocation.Location)
                {
                        ...
                        foreach (Int64 lvTrainId in mStopLocationOcupation[lvGene.StopLocation.Location])
                        {
                            ...
                            if ((lvGene.Track == lvGen.Track) && lvGene.TrainId != lvGen.TrainId)
                            {
                                ...
                                if (DebugLog.EnableDebug)
                                {
                                    ...
                                    lvStrInfo.Clear();
                                    lvStrInfo.Append("ProcessGene => pGene.StopLocation.Location == pGene.StartStopLocation.Location(");
                                    lvStrInfo.Append(lvGene.ToString());
                    ...
                    if (DebugLog.EnableDebug)
                    {
                        ...
                        lvStrInfo.Clear();
                        lvStrInfo.Append("ProcessGene => Gene aceito por pGene.StopLocation.Location == pGene.StartStopLocation.Location(");
                        lvStrInfo.Append(lvGene.ToString());
                        lvStrInfo.Append(")");
            ...
            if (lvNextStopLocation == null)
            {
                ...
                if (DebugLog.EnableDebug)
                {
                    ...
                    lvStrInfo.Append(lvGene.ToString());
                    lvStrInfo.Append(") ! ProcessGene => lvNextStopLocation == null");
            ...
            if ((lvStopLocation == lvEndStopLocation) && (lvStopLocation != null))
            {
                ...
                if (DebugLog.EnableDebug)
                {
                    ...
                    lvStrInfo.Append("Gene removido de mDicTrain(");
                    lvStrInfo.Append(lvGene.ToString());
                    lvStrInfo.Append(") ! ProcessGene => (lvStopLocation == lvEndStopLocation) && (lvStopLocation != null)");
            ...
            if (lvGene.Direction > 0)
            {
                if (lvStopLocation != null)
                {
                    if (lvStopLocationValue >= lvEndStopLocation.Location)
                    {
                        ...
                        if (DebugLog.EnableDebug)
                        {
                            ...
                            lvStrInfo.Append("Gene removido de mDicTrain(");
                            lvStrInfo.Append(lvGene.ToString());
                            lvStrInfo.Append(") ! ProcessGene => lvStopLocation.Location >= lvEndStopLocation.Location");
                ...
                else if (lvGene.Coordinate >= lvEndStopLocation.Start_coordinate)
                {
                    ...
                    if (DebugLog.EnableDebug)
                    {
                        ...
                        lvStrInfo.Append("Gene removido de mDicTrain(");
                        lvStrInfo.Append(lvGene.ToString());
                        lvStrInfo.Append(") ! ProcessGene => lvGene.Coordinate >= lvEndStopLocation.Start_coordinate");
            ...
            else
            {
                ...
                if (lvGene.Coordinate <= lvEndStopLocation.End_coordinate)
                {
                    ...
                    if (DebugLog.EnableDebug)
                    {
                        ...
                        lvStrInfo.Append("Gene removido de mDicTrain(");
                        lvStrInfo.Append(lvGene.ToString());
                        lvStrInfo.Append(") ! ProcessGene => lvGene.Coordinate <= lvEndStopLocation.End_coordinate");
            ...
            if (DebugLog.EnableDebug)
            {
                ...
                DebugLog.Logar(" ", pIndet: TrainIndividual.IDLog);
                DebugLog.Logar(" --------- ProcessGene ---------- ", pIndet: TrainIndividual.IDLog);
            ...
            if ((lvGene.Track <= lvNextCapacity) && (((lvGene.Direction > 0) && ((lvNextSwitch == null) || (lvNextStopLocation.Start_coordinate < lvNextSwitch.Start_coordinate))) || ((lvGene.Direction < 0) && ((lvNextSwitch == null) || (lvNextStopLocation.End_coordinate > lvNextSwitch.End_coordinate)))))
            {
                ...
                lvHeadwaysTime = new List<DateTime>();
                SetHeadwaySetTime(lvGene, lvStopLocation, lvNextStopLocation, ref lvHeadwaysTime, true);
            ...
            else
            {
                ...
                else
                {
                    lvDestTrack = 0;
                    lvHeadwaysTime = new List<DateTime>();
                    SetHeadwaySetTime(lvGene, lvStopLocation, lvNextStopLocation, ref lvHeadwaysTime);
                }
            }
            ...
            if (Interdicao.GetList().Count > 0)
            {
                if (lvGene.Direction > 0)
                {
                    if (lvStopLocation != null)
                    {
                        lvInterdition = GetInterdiction(lvStopLocation.End_coordinate, lvNextStopLocation.Start_coordinate, lvGene.Time, 1);
                    }
                    else
                    {
                        lvInterdition = GetInterdiction(lvGene.Coordinate, lvNextStopLocation.Start_coordinate, lvGene.Time, 1);
                    }
                }
                else
                {
                    if (lvStopLocation != null)
                    {
                        lvInterdition = GetInterdiction(lvNextStopLocation.End_coordinate, lvStopLocation.Start_coordinate, lvGene.Time, 1);
                    }
                    else
                    {
                        lvInterdition = GetInterdiction(lvNextStopLocation.End_coordinate, lvGene.Coordinate, lvGene.Time, 1);
                    }
                }
            ...
            if (lvNextCapacity > 0)
            {
                /* - Assumir que o trem não para e verificar em lvHeadwaysTime se existe condição de parada ou 
                *    oportunidade de seguir sem parar.
                *  - Verificar em lvHeadwaysTime de forma decrescente para poder quebrar o loop mais rapidamente
                */
                ...
                if (lvGene.Speed == 0.0)
                {
                    lvMeanSpeed = mMinSpeedLimit;
                }
                else
                {
                    lvDistance = Math.Abs(lvEndCoordinate - lvInitCoordinate);
                    lvMeanSpeed = (lvDistance / 100000.0) / lvSpentTime;

                    if (lvMeanSpeed > mVMA)
                    {
                        lvMeanSpeed = mVMA;
                    }
                    else if (lvMeanSpeed < mMinSpeedLimit)
                    {
                        lvMeanSpeed = mMinSpeedLimit;
                    }
                    else
                    ...
                    if (DebugLog.EnableDebug)
                    {
                        DebugLog.Logar("lvMeanSpeed = " + lvMeanSpeed, pIndet: TrainIndividual.IDLog);
                    }
#endif

                    lvDepTime = lvGene.Time.AddHours(lvSpentTime);
                    if (lvGene.Direction > 0)
                    {
                        lvDistance = lvNextStopLocation.Start_coordinate - lvEndCoordinate + mTrainLen;
                    }
                    else
                    {
                        lvDistance = lvEndCoordinate - lvNextStopLocation.End_coordinate + mTrainLen;
                    }

                    lvArrTime = lvDepTime.AddHours((lvDistance / 100000.0) / lvMeanSpeed);

                    for(int ind = lvHeadwaysTime.Count-1; ind >= 0; ind--)
                    {
                        if((lvHeadwaysTime[ind] >= lvDepTime) && (lvHeadwaysTime[ind] >= lvArrTime))
                        {
                            /* Precisa recalcular média e lvSpentTime */

                            lvStayTime = (lvHeadwaysTime[ind] - lvGene.Time).TotalHours;
                    ...
                    if ((lvMeanSpeed <= mMinSpeedLimit) && (lvGene.Time > lvGene.DepartureTime))
                    {
#if DEBUG
                        if (DebugLog.EnableDebug)
                        {
                ...
                if (mCalcDistRef)
                {
                    if (!mDicDistRef.ContainsKey(lvNewGene.GetID()))
                    {
                        mDicDistRef.Add(lvNewGene.GetID(), mList.Count);
                    }
                }
                ...
                {
                    lvGenesStopLocation = mStopLocationDeparture[lvNewGene.StopLocation.Location];
                    if (lvGene.Track <= lvNextAvailable.Length)
                    {
                ...
                */
                ...
                if (lvNewGene.StopLocation != null)
                {
                    lvGenesStopLocation = mStopLocationArrival[lvNewGene.StopLocation.Location];
                    if (lvNewGene.Track <= lvNextAvailable.Length)
                    {
                        lvGenesStopLocation[(lvNewGene.Track - 1) * 2 + Math.Max(0, (int)lvNewGene.Direction)] = lvNewGene;

                        DumpStopArrivalLocation(lvNewGene.StopLocation);
                    }
                }
                ...
                if (lvNextStopLocation != null)
                {
                    ...
                    else
                    {
                        ...
                        if (DebugLog.EnableDebug)
                        {
                            ...
                            lvStrInfo.Append(") ! ProcessGene => else de ((lvNewGene.Direction > 0) && (lvNextStopLocation.Location < lvEndStopLocation.Location)) || ((lvNewGene.Direction < 0) && (lvNextStopLocation.Location > lvEndStopLocation.Location))");
                            DebugLog.Logar(lvStrInfo.ToString(), pIndet: TrainIndividual.IDLog);
                ...
                if (DebugLog.EnableDebug)
                {
                    ...
                    lvStrInfo.Clear();
                    lvStrInfo.Append("Gene Adicionado ");
                    lvStrInfo.Append(mList[mList.Count - 2].TrainId);
                    lvStrInfo.Append(" (");
                    lvStrInfo.Append(mList[mList.Count - 2].TrainName);
                    lvStrInfo.Append(") em ");
                    lvStrInfo.Append(mList[mList.Count - 2].SegmentInstance.Location);
                    lvStrInfo.Append(".");
                    lvStrInfo.Append(mList[mList.Count - 2].SegmentInstance.SegmentValue);
                    lvStrInfo.Append(" partindo em ");
                    lvStrInfo.Append(mList[mList.Count - 2].Time);
                    lvStrInfo.Append(" => ");
                    lvStrInfo.Append(mList[mList.Count-1].TrainId);
                    lvStrInfo.Append(" (");
                    lvStrInfo.Append(mList[mList.Count - 1].TrainName);
                    lvStrInfo.Append(") em ");
                    lvStrInfo.Append(mList[mList.Count - 1].SegmentInstance.Location);
                    lvStrInfo.Append(".");
                    lvStrInfo.Append(mList[mList.Count - 1].SegmentInstance.SegmentValue);
                    lvStrInfo.Append(" com HeadWayTime de ");
                    lvStrInfo.Append(mList[mList.Count - 1].HeadWayTime);
            ...
            if (pGene == null)
            {
                foreach (Gene lvGene in mDicTrain.Values)
                {
                    ...
                    if (lvCount > 1)
                    {
                        if (lvGene.StopLocation == null)
                        {
                            DebugLog.Logar("Stop Locations Count para (" + lvGene + ") = " + lvCount, pIndet: TrainIndividual.IDLog);
                        }
                        else
                        {
                            DebugLog.Logar("Stop Locations Count para (" + lvGene + ", Current: " + lvGene.StopLocation + ") = " + lvCount, pIndet: TrainIndividual.IDLog);
                        }
                    }
#endif
                }
    ...
    {
        string lvSerializationFile = ConfigurationManager.AppSettings["LOG_PATH"] + "Data\\" + "Individual_" + mUniqueId + ".bin";

        if (Directory.Exists(ConfigurationManager.AppSettings["LOG_PATH"] + "Data\\"))
        {
    ...
    public void UnSerialize()
    {
        ...
        if (File.Exists(lvSerializationFile))
        {
            using (Stream stream = File.Open(lvSerializationFile, FileMode.Open))
            {
                ...
                mList = (List<Gene>)bformatter.Deserialize(stream);
            }
        }
    }

    public static List<Gene> UnSerialize(int pUniqueId)
    {
        List<Gene> lvRes = null;

        string lvSerializationFile = ConfigurationManager.AppSettings["LOG_PATH"] + "Data\\" + "Individual_" + pUniqueId + ".bin";
        ...
        if (File.Exists(lvSerializationFile))
        {
            using (Stream stream = File.Open(lvSerializationFile, FileMode.Open))
            {
                ...
                lvRes = (List<Gene>)bformatter.Deserialize(stream);
            }
        }
    ...
    public void AddGeneRef(Gene pGene)
    {
        if(!mDicTrain.ContainsKey(pGene.TrainId))
        {
            mDicTrain.Add(pGene.TrainId, pGene);
        }
    }
